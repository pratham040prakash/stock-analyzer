...
import { DAY_PNL_REFRESH_MS } from "@/lib/useDayPnlPoll";

type MonitorDayPnlResponse = {
  dayPnl: number | null;
};
...
export function useMonitorDayPnlPoll({ enabled }: Options) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const res = await apiFetch("/api/monitor/day-pnl", {
        method: "GET",
        cache: "no-store",
      });
      const data = await parseApiJson<MonitorDayPnlResponse>(res, "Monitor Day P&L");
